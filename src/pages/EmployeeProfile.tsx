...
const statCards = {
    understanding: 82,
    understandingDelta: '+5% с прошлого месяца',
...
export default function EmployeeProfile() {
    ...
    const avatarSrc =
        typeof employee.avatar === 'string' && employee.avatar.trim().length > 0 && employee.avatar !== initials
            ? employee.avatar
            : exampleAvatar;

    const firstName = employee.name.split(' ')[0] || 'Иван';
    ...
    return (
        <PersonaLearnLayout>
            <div className="mx-5 my-6 space-y-7 xl:mx-6 xl:my-7">
                ...
                <section
                    className="rounded-[24px] border border-slate-200 bg-white px-7 py-7 shadow-[0_2px_10px_rgba(15,23,42,0.04)]">
                    <div className="flex flex-col gap-6 xl:flex-row xl:items-center xl:justify-between">
                        <div className="flex min-w-0 flex-col gap-5 sm:flex-row sm:items-center">
                            <div
                                className="flex h-[126px] w-[126px] shrink-0 items-center justify-center overflow-hidden rounded-[20px] bg-slate-100 text-[34px] font-black text-slate-500 shadow-inner">
                                {avatarSrc ? (
                                    <img
                                        src={avatarSrc}
                                        alt={employee.name}
                            ...
                            <div className="min-w-0 space-y-4">
                                <div>
                                    ...
                                    <p className="mt-2 text-[20px] font-semibold leading-7 text-slate-500">
                                        {employee.position || 'Sales Lead'}
                                    </p>
                                </div>

                                <div
                                    className="flex flex-wrap items-center gap-4 text-[14px] font-semibold text-slate-400">
                  <span className="inline-flex items-center gap-2">
                    <SvgIcon name="calendar" className="h-4 w-4"/>
                    <span>Добавлен: 12.05.2023</span>
                  </span>
                                    <span
                                        className="rounded-full bg-blue-50 px-4 py-2 text-[14px] font-bold text-blue-600">
                    АНАЛИТИК / ДРАЙВЕР
                  </span>
                                </div>
                            </div>
                        ...
                        <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
                            <button
                                className="inline-flex h-[56px] items-center justify-center gap-3 rounded-[16px] bg-blue-600 px-7 text-[16px] font-bold text-white transition hover:bg-blue-700">
                ...
                <section className="space-y-4">
                    <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                        <div className="flex items-center gap-3">
                            <div
                                className="flex h-8 w-8 items-center justify-center rounded-[8px] bg-blue-600 text-white">
                    ...
                    <div className="grid grid-cols-1 gap-6 xl:grid-cols-[minmax(0,1fr)_360px]">
                        <div className="grid grid-cols-1 gap-5 lg:grid-cols-[280px_minmax(0,1fr)]">
                            <article
                                className="rounded-[24px] border border-slate-200 bg-white px-8 py-7 shadow-[0_2px_10px_rgba(15,23,42,0.04)]">
                                ...
                                <div className="mt-7 flex items-center justify-center">
                                    ...
                                    >
                                        <div
                                            className="flex h-[106px] w-[106px] items-center justify-center rounded-full bg-white text-[34px] font-black text-slate-900">
                            ...
                            <article
                                className="rounded-[24px] border border-slate-200 bg-white px-6 py-7 shadow-[0_2px_10px_rgba(15,23,42,0.04)]">
                                        ...
                                        <ul className="space-y-4 text-[16px] font-medium leading-9 text-slate-700">
                                            {statCards.gaps.map((item) => (
                                                <li key={item} className="flex gap-3">
                                                    <span
                                                        className="mt-[14px] h-2.5 w-2.5 shrink-0 rounded-full bg-red-400"/>
                                                    <span>{item}</span>
                                                </li>
                                            ))}
                                        </ul>
                                    ...
                                    <div className="space-y-4 md:pl-8">
                                        ...
                                        <ul className="space-y-4 text-[16px] font-medium leading-9 text-slate-700">
                                            {statCards.strengths.map((item) => (
                                                <li key={item} className="flex gap-3">
                                                    <span
                                                        className="mt-[14px] h-2.5 w-2.5 shrink-0 rounded-full bg-emerald-400"/>
                                                    <span>{item}</span>
                                                </li>
                                            ))}
                                        </ul>
                        ...
                        <aside className="space-y-6">
                            <article
                                className="rounded-[24px] border border-slate-200 bg-white px-7 py-7 shadow-[0_2px_10px_rgba(15,23,42,0.04)]">
                                <h3 className="text-[24px] font-black tracking-[-0.03em] text-slate-900">Быстрые
                                    действия</h3>
                                <div className="mt-6 space-y-4">
                                    <button
                                        className="flex h-[56px] w-full items-center justify-center gap-3 rounded-[16px] bg-blue-600 px-5 text-[16px] font-bold text-white transition hover:bg-blue-700">
                                        <SvgIcon name="plus-circle" className="h-5 w-5"/>
                                        Назначить обучение
                                    </button>
                                    <button
                                        className="flex h-[56px] w-full items-center justify-center gap-3 rounded-[16px] bg-blue-50 px-5 text-[16px] font-bold text-blue-600 transition hover:bg-blue-100">
                                        <SvgIcon name="refresh" className="h-5 w-5"/>
                                        Переназначить материал
                                    </button>
                                    <button
                                        className="flex h-[56px] w-full items-center justify-center gap-3 rounded-[16px] border border-slate-200 bg-white px-5 text-[16px] font-bold text-slate-900 transition hover:bg-slate-50">
                            ...
                            <article
                                ...
                                <div className="flex items-start justify-between gap-4">
                                    <h3 className="text-[24px] font-black tracking-[-0.03em] text-slate-900">Профиль
                                        DISC</h3>
                                    <div
                                ...
                                <div className="mt-6 space-y-5">
                                    {discMetrics.map((item) => (
                                        <div key={item.key} className="space-y-2.5">
                                            <div
                                                className="flex items-center justify-between gap-3 text-[14px] font-black uppercase">
                                ...
                                <blockquote
                                    className="mt-6 rounded-[16px] bg-slate-50 px-5 py-4 text-[16px] italic leading-8 text-slate-500">
                ...
                <section className="grid grid-cols-1 gap-6 xl:grid-cols-[minmax(0,1fr)_360px]">
                    <article
                        className="overflow-hidden rounded-[24px] border border-slate-200 bg-white shadow-[0_2px_10px_rgba(15,23,42,0.04)]">
